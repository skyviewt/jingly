...
var timelineHome = new TimelineMax({ pause: true });
var timelineMcgill = new TimelineMax({ pause: true });
var timelineLaronde = new TimelineMax({ pause: true });
var timelineMileEnd = new TimelineMax({ pause: true });
...
function readyFn(jQuery) {
  $.mobile.loading().hide();
  // get background svg
  var svgUrl = "../experience/experience.svg";
  ...
  window.addEventListener(
    ...
    function() {
      ...
      TweenMax.to(PARTS.SquareGroup.node, 0.5, {
        x: $(window).height() / 5 * currentDot - DIMENSION.SquareLength / 2
      });
    },
    false
  );
  ...
  $(document).on("keydown", function(event) {
    ...
    switch (keyCode) {
      ...
      case 38:
        // up
        var currenSectiontIndex = $.inArray(
          sliderActiveTag[currentDot],
          sliderNames[currentDot]
        );
        if (currenSectiontIndex !== -1 && currenSectiontIndex > 0) {
          showSection(
            sliderNames[currentDot][currenSectiontIndex - 1].substr(1)
          );
        }
        break;
      case 40:
        //down
        var currenSectiontIndex1 = $.inArray(
          sliderActiveTag[currentDot],
          sliderNames[currentDot]
        );
        if (
          currenSectiontIndex1 !== -1 &&
          currenSectiontIndex1 < sliderNames[currentDot].length - 1
        ) {
          showSection(
            sliderNames[currentDot][currenSectiontIndex1 + 1].substr(1)
          );
        }
        break;
    }
...
function changeSlide(index, event) {
  ...
  if (event === undefined) {
    squarePosition =
      $(window).width() / 5 * index -
      DIMENSION.SquareLength / 2 +
      ($(window).width() < 760 ? 7 : 10);
  } else {
    squarePosition = event.pageX - DIMENSION.SquareLength / 2;
  }

  if (!isAnimating) {
    TweenMax.to(PARTS.timelineInside, 1, {
      width: timelineWidth,
      ease: Back.easeInOut,
      delay: 0.3
    });
    TweenMax.to(PARTS.SquareGroup.node, 1, {
      x: squarePosition,
...
function makeSquare(isInit) {
  DIMENSION.SquareLength = Math.min(
    Math.floor(Math.max($(window).width(), $(window).height()) / 20),
    50
  );
  ...
  if (isInit)
    TweenMax.to(PARTS.SquareGroup.node, 1, {
      x: $(window).width() * 0.2 - DIMENSION.SquareLength / 2,
      ease: Back.easeInOut
    });
...
function animateDotChange() {
  TweenMax.to(PARTS["dot" + prevDot], 0.2, {
    scale: 1,
    ease: Back.easeInOut,
    delay: 0.5
  });
  TweenMax.to(PARTS["dot" + currentDot], 0.2, {
    scale: 0,
...
function hidePrevAccesory() {
  var sign = prevDot < currentDot ? -1 : 1;
  if (prevDot === 4) {
    ...
    //skills
    TweenMax.fromTo(
      [PARTS.WorkerHat.node, PARTS.WorkerHatEdge.node],
      0.5,
      { x: 0, opacity: 1 },
      {
        x: sign * DIMENSION.SquareLength / 2,
        opacity: 0,
        ease: Power2.easeOut,
        onComplete: function onComplete() {
          showCurrentSlideAndAcc();
...
function showCurrentSlideAndAcc() {
  ...
  if (currentDot === 4) {
    ...
    if (timelineMcgill.getChildren(true, true, true, 0).length === 0) {
      timelineMcgill.add(
        [
          TweenMax.to("#mcgill", 1, {
            scale: 1.05,
            transformOrigin: "center bottom",
            ease: Sine.easeInOut
          }),
          TweenMax.to("#mcgill-flag", 0.8, {
            skewY: -10,
            scaleX: 0.85,
    ...
    TweenMax.fromTo(
      [PARTS.WorkerHat.node, PARTS.WorkerHatEdge.node],
      0.5,
      { x: sign * DIMENSION.SquareLength, opacity: 0 },
      ...
      timelineMileEnd.add(
        [
          ...
          TweenMax.staggerFromTo(
            ".graffitis",
            0.8,
            { oppacity: 0, rotation: 125, scale: 1.2 },
            {
              opacity: 1,
              rotation: 0,
              scale: 1,
...
function toggleSlide() {
  ...
  TweenMax.fromTo(
    "#slider-" + prevDot + sliderActiveTag[prevDot],
    0.4,
    { x: 0, opacity: 1 },
    {
      x: -1 * sign * 80,
      ...
      onComplete: function onComplete() {
        ...
        TweenMax.fromTo(
          "#slider-" + currentDot + sliderActiveTag[currentDot],
          0.6,
          { x: sign * 80, opacity: 0 },
          {
            x: 0,
...
function toggleSliderNav(sectionName) {
  if (sectionName === undefined) {
    $("#slider-" + currentDot + sliderActiveTag[currentDot]).toggleClass(
      "hide"
    );
    $("#slider-nav-section" + sliderActiveTag[currentDot]).toggleClass(
      "active"
    );
  } else {
